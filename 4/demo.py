...
def get_none_jkey(key=None, value=None):
    # key = "IgpvzINcuA"
    # value = "stg0l85Aq1"
    text = base64.b64encode((key + value).encode('utf-8')).decode()
    md = hashlib.md5(text.replace("=", "").encode('utf-8'))
    j_key = md.hexdigest()
...
def req(page):
    ...
    cookies = {
        "Hm_lvt_9bcbda9cbf86757998a2339a0437208e": "1705130511,1705750038,1705795579,1705826155",
        "qpfccr": "true",
        "no-alert3": "true",
...
def get_data(items):
    ...
    key = items['key']
    value = items['value']
    none_number = get_none_jkey(key, value)
    # print(none_number)
    ...
    for i in tree:
        ...
        for j in i.xpath('./img'):
            if (none_number not in j.xpath('./@class')[0]):
                style = float(re.findall(r'left:(.*?)px', j.xpath('./@style')[0])[0])
                image_list.append([get_image()[j.xpath('./@src')[0]], style])
        # print(image_list)

        num = image_list.copy()
        for n, sty in enumerate(image_list):
            # print(n, sty)
            index = int(sty[1] / 11.5)
            if (index != 0):
                num[n + index] = image_list[n]

        s = ''.join([str(number[0]) for number in num])
